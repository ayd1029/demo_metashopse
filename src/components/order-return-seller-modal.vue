<template>
  <q-modal v-model="orderReturnModal" position="" :content-css="{minWidth: '40vw'}">

    <q-modal-layout>
      <q-toolbar slot="header" color="black" inverted>
        <q-btn flat round dense icon="loop" />
        <q-toolbar-title>반품교환 정보</q-toolbar-title>
      ...
      <q-list v-for="orderProductVo in orderProductList" :key="orderProductVo.seq">
        <table align="center" border="0" width="95%">
          ...
          <tr>
            <td align="right"><q-select inverted color="primary" v-model="orderProductVo.status_return" :options="statusOptions" style="width:75px; height:30px;" dense readonly/></td>
          </tr>
          ...
          <tr v-if="orderProductVo.status_return === $store.state.ORDER_STATUS_REQUEST_EXCHANGE || orderProductVo.status_return === $store.state.ORDER_STATUS_REQUEST_RETURN">
            <td align="left" colspan="2"><q-input v-model.trim="orderProductVo.comment_return" stack-label="요청사항" color="white" inverted-light type="textarea" rows="2" readonly /></td>
          </tr>
        </table>
      </q-list>
      <br>
      ...
      <q-toolbar color="white" >
        <q-btn :loading="loading" :percentage="percentage" color="primary" @click="requestReturn" icon="done" style="width:100%; height:50px;">
          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;저장하기
          <span slot="loading">
