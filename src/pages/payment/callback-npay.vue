...
export default {
  ...
  created: function () {
    ...
    // 결제 콜백 처리
    this.callbackPayment(resultCd, resultMsg, tid)
  },
  mounted: function () {
  },
  methods: {
    // 결제 콜백
    callbackPayment (resultCd, resultMsg, tid) {
      console.log('resultCd: ' + resultCd + ' , resultMsg: ' + resultMsg + ' , tid: ' + tid)

      if ((resultCd === 'Success') && tid !== '') { // 결제 성공시 처리
        // 결제승인결과 확인
        ...
        this.$axios.get(this.$store.state.apiServerIp + '/api/payment/getOrdersInfoFromSession')
          .then((result) => {
            ...
            this.$axios.get(this.$store.state.apiServerIp + '/api/payment/checkNpayIsCompleted', {params: {tid: tid}})
              .then((result) => {
                ...
                if (result.data === 0) { // tid에 해당하는 결제성공 건수가 0건이면 이상처리
                  ...
                  this.goProductDetailAgain()
                } else { // 결제 정상처리
                  // 장바구니에서 주문한 경우 - 장바구니에서 해당 상품 리스트 삭제
                  if (this.ordersVo.savedFromCartFlag) {
    ...
    // 결제 실패시 결제시도한 상품상세화면으로 이동
    goProductDetailAgain () {
      // 상품상세화면 패스 설정
      let path = this.$cookie.get('PAYMENT_REDIRECT_URL') // 상품상세 페이지
      // 상품상세화면으로 이동
