                    ...
                    <q-btn flat round dense color="grey" icon="more_vert">
                      <q-popover>
                        <q-list link class="scroll" style="min-width: 100px">
                          ...
                          <!-- <q-item key="2" v-if="contentsVo.reg_id === $store.state.userVo.uid" v-close-overlay @click.native="goContensModify(contentsVo)"><q-item-main label="수정하기" /></q-item> -->
                          <!-- <q-item key="3" v-if="contentsVo.reg_id === $store.state.userVo.uid" v-close-overlay @click.native="deleteContents(contentsVo, index)"><q-item-main label="삭제하기" /></q-item> -->
...
export default {
  ...
  watch: {
    '$route.query.seq': function (seqContents) {
      this.selectContents(seqContents)
    }
  },
  created: function () {
    ...
    // 컨텐츠 조회
    this.selectContents(this.$route.query.seq)
