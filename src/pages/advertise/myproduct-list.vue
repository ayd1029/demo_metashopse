  ...
  <q-page class="">
    ...
    <q-layout-header :reveal="$store.state.device !== 'P'">
      <q-toolbar color="black" inverted>
        ...
        <q-toolbar-title>광고상품 목록</q-toolbar-title>
        ...
        <q-list style="padding:0px 0px 0px 0px" v-for="(item, index) in productList" :key="item.seq">
          <q-item link style="padding:0px 0px 0px 0px">
            <table border="0" style="width: 100%; padding:5px 0px 5px 0px;">
              ...
              <tr>
                <td colspan="2"><a @click="goProductDetail(item.seq_product)"><font size="4" color="black" style="font-weight:bold;">{{ addComma(item.price) }}&nbsp;원</font></a></td>
                <td colspan="3" style="text-align: right;"><a @click="goProductDetail(item.seq_product)"><font size="2" color="black">{{ addComma(item.reward_coin) }} <img src="statics/images/logo/logocoin.png" style="width: 9px">&nbsp;&nbsp;&nbsp;</font></a></td>
              </tr>
              <tr>
  ...
  methods: {
    ...
    selectListMax () {
      this.$axios.get(this.$store.state.apiServerIp + '/api/myproduct/selectMyproductListLastPageNum',
        {params: {pageSize: this.pageSize, uid: this.$store.state.userVo.uid}})
        .then((result) => {
    ...
    selectList (idx, done) {
      this.$axios.get(this.$store.state.apiServerIp + '/api/myproduct/selectMyproductList',
        {params: {pageNum: idx, pageSize: this.pageSize, uid: this.$store.state.userVo.uid}})
        .then((result) => {
          // console.log(JSON.stringify(result.data))
    ...
    deleteProduct (index) {
      ...
      this.$q.dialog({
        ...
        this.$axios.post(this.$store.state.apiServerIp + '/api/myproduct/deleteMyproduct', this.productList[index])
          .then((result) => {
            // console.log(JSON.stringify(result.data))
