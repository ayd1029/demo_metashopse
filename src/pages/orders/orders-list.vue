<template>
  <q-page class="">
    ...
    <reviewTargetModal ref="reviewTargetModal" />
    <vbankDepositInfoModal ref="vbankDepositInfoModal" />
    ...
    <q-layout-header :reveal="$store.state.device !== 'P'">
      <q-toolbar color="black" inverted>
        ...
        <q-toolbar-title>주문/배송조회</q-toolbar-title>
        ...
        <q-list v-for="(orders, index) in ordersList" :key="orders.seq">
          <table border="0" align="center" style="width: 95%">
            ...
            <tr>
              ...
              <td colspan="3" align="right">
                <font size="2" color="grey" @click="showOrderNo(orders)">주문번호보기</font>
                <!--<q-btn dense color="blue" label="주문상세보기" @click="132" outline size="md" flat />-->
              </td>
            </tr>
            <tr>
              ...
              <td v-if="orders.status === $store.state.ORDER_STATUS_PAYMENT_WAIT"><!-- 입금대기 상태일 경우 입금정보 표시 -->
                <q-btn dense color="red" label="입금정보" @click="showVbankDepositInfoModal(orders.tid)" outline size="md" style="width:70px" />
              </td>
              <td v-else></td>
              <td colspan="3" align="right"><font size="2" color="grey">{{ deliveryFormatDate(orders.delivery_end_date) }}</font></td>
            </tr>
            <tr>
              <td colspan="5"></td>
            </tr>
          ...
          <div v-for="item in orderProductList" :key="item.seq">
            ...
            <div v-if="orders.order_no === item.order_no">
              <table border="0" align="center" style="width: 95%">
                ...
                <tr>
                  <td colspan="1" rowspan="2" align="center">
                    <a @click="goProductDetail(item.seq_product)"><img :src="item.image_url" style="width:50px; height:50px; object-fit:cover; border-radius: 3px;"></a>
                  </td>
              ...
              <table v-if="orders.status !== $store.state.ORDER_STATUS_CANCEL &&
                orders.status !== $store.state.ORDER_STATUS_CANCEL_SELLER &&
                item.showButtonYn === 'Y'" border="0" align="center" style="width: 95%">
                <thead>
                  <th width="20%"></th>
                  <th width="20%"></th>
                  <th width="20%"></th>
                ...
                <tr>
                  <td colspan="1" align="center">
                    <q-btn v-if="orders.status < $store.state.ORDER_STATUS_DELIVERY_WAIT &&
                      orders.seller_cnt === '1' &&
                      (orders.pay_type === 'wcard' || orders.pay_type === 'Card')"
                      label="주문취소"
                      @click="cancelOrders(index)" size="md" dense color="primary" style="width:70px" />
                  </td>
                  <td colspan="1" align="right">
                    <q-btn v-if="orders.status >= $store.state.ORDER_STATUS_DELIVERY_WAIT &&
                      orders.status !== $store.state.ORDER_STATUS_PURCHASE_CONFIRMED"
                      label="구매확정"
                      @click="completeOrders(index)" size="md" dense color="green" style="width:70px" />
                 </td>
                  <td colspan="2" align="right">
                    <q-btn v-if="orders.status >= $store.state.ORDER_STATUS_DELIVERY_WAIT &&
                      orders.status !== $store.state.ORDER_STATUS_PURCHASE_CONFIRMED &&
                      orders.request_return_yn !== 'Y'"
                      label="반품교환"
                      @click="requestReturn(index)" size="md" dense color="primary" style="width:70px" />
                  </td>
                  <td colspan="1" align="right">
                    <q-btn v-if="orders.delivery_no != null" dense color="blue" label="배송조회"
                      @click="showDeliveryTracking(index)" size="md" style="width:70px" />
                  </td>
                </tr>
              </table>
            </div>
          </div>
          <table v-if="orders.status === $store.state.ORDER_STATUS_PURCHASE_CONFIRMED" style="width:95%;" align="center">
            <tr></tr>
            <tr></tr>
            <tr></tr>
            <tr>
              <q-btn outline color="primary" @click="showReviewTargetModal(index)" icon="create" label="구매후기" style="width:100%; height: 35px" />
            </tr>
          </table>
        </q-list>
        <br>
...
export default {
  ...
  methods: {
    ...
    // 상품 리스트 마지막 페이지 조회
    selectListMax () {
      this.$axios.get(this.$store.state.apiServerIp + '/api/orders/selectOrdersListLastPageNum',
    ...
    // 상품 리스트 조회
    selectList (idx, done) {
      this.$axios.get(this.$store.state.apiServerIp + '/api/orders/selectOrdersList',
    ...
    showOrderReturnSellerModal (orderNo) {
      ...
      for (let i = 0; i < this.orderProductList.length; i++) {
        let orderProductRow = this.orderProductList[i]
        if (orderNo === orderProductRow.order_no) { // 주문번호가 같으면 추가
    ...
    modifyListTo (orderNo, type) {
      ...
      for (let i = 0; i < this.orderProductList.length; i++) {
        let orderProductRow = this.orderProductList[i]
        if (orderNo === orderProductRow.order_no) { // 주문번호가 같으면 상태 수정
      ...
      for (let i = 0; i < this.ordersList.length; i++) {
        let ordersRow = this.ordersList[i]
        if (orderNo === ordersRow.order_no) { // 주문번호가 같으면 상태 수정
    ...
    // 구매후기 작성대상 모달 표시
    showReviewTargetModal (index) {
      let productList = [] // 구매후기 작성대상 모달화면에 표시할 주문상품 리스트
      let selectedOrderNo = this.ordersList[index].order_no // 선택된 주문번호
      for (let i = 0; i < this.orderProductList.length; i++) {
        if (selectedOrderNo === this.orderProductList[i].order_no) {
          // productList.push(JSON.parse(JSON.stringify(this.orderProductList[i]))) // 주소참조면 저장을 안해도 주문목록 화면이 설정되어버림
          productList.push(this.orderProductList[i]) // review-target-modal 화면에서 직접 수정하지 않기 때문에 주소참조
        }
      }
      this.$refs.reviewTargetModal.orderProductList = productList
      this.$refs.reviewTargetModal.show() // 구매후기 모달 열기
    },
    callbackReviewTarget () {
    },
    // 주문상태가 입금대기(00)인 경우 가상계좌 입금정보 화면 표시
    showVbankDepositInfoModal (tid) {
      // alert(tid)
      this.$refs.vbankDepositInfoModal.tid = tid // 결제 거래ID
      this.$refs.vbankDepositInfoModal.show() // 배송관리 모달 열기
    },
    ...
    goBack () {
      if (this.$store.state.doNotBack) { // 뒤로가기 허용하지 않는 화면에서 온 경우
        this.$store.state.doNotBack = false
