<template>
  <q-page class="">
    ...
    <table width="94%" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td align="center">
          <q-btn dense color="red" icon="play_arrow" @click="showYoutubeChannel" size="md">&nbsp;&nbsp;&nbsp;&nbsp;유튜브 채널 보기&nbsp;&nbsp;&nbsp;&nbsp;</q-btn>
        </td>
      </tr>
      <tr>
        <td align="center">&nbsp;</td>
...
export default {
  ...
  data () {
    return {
      ...
      title: decodeURI(this.$route.query.name), // 화면 타이틀
      kwd1: encodeURI(this.$route.query.kwd1),
      kwd2: encodeURI(this.$route.query.kwd2),
      kwd3: encodeURI(this.$route.query.kwd3),
      kwd4: encodeURI(this.$route.query.kwd4),
      kwd5: encodeURI(this.$route.query.kwd5),
  ...
  created: function () {
    // alert(this.$route.query.kwd1)
    // 검색 키워드 파라미터
    /*
    let kwd1 = encodeURI(this.$route.query.kwd1)
    let kwd2 = encodeURI(this.$route.query.kwd2)
    let kwd3 = encodeURI(this.$route.query.kwd3)
    let kwd4 = encodeURI(this.$route.query.kwd4)
    let kwd5 = encodeURI(this.$route.query.kwd5)
    */
    // alert(this.kwd1)
  ...
  methods: {
    ...
    selectContentsListMax () {
      this.$axios.get(this.$store.state.apiServerIp + '/api/youtuber/selectYoutuberContentsListLastPageNum',
        {params: {pageSize: this.pageSizeContents, kwd1: this.kwd1, kwd2: this.kwd2, kwd3: this.kwd3, kwd4: this.kwd4, kwd5: this.kwd5}})
        .then((result) => {
    ...
    selectContentsList (idx, done) {
      this.$axios.get(this.$store.state.apiServerIp + '/api/youtuber/selectYoutuberContentsList',
        {params: {pageNum: idx, pageSize: this.pageSizeContents, kwd1: this.kwd1, kwd2: this.kwd2, kwd3: this.kwd3, kwd4: this.kwd4, kwd5: this.kwd5}})
        .then((result) => {
          // console.log(JSON.stringify(result.data))
    ...
    // 유튜브 채널 오픈 - 앱
    showYoutubeChannel () {
      let channelUrl = this.$store.state.youtubeChannelUrl.replace('https://', '')
      if (this.$store.state.device === 'android') {
        window.open('intent://' + channelUrl + '#Intent;scheme=http;package=com.google.android.youtube;end', '_blank')
      } else if (this.$store.state.device === 'ios') {
        window.open('youtube://' + channelUrl, '_blank')
      } else {
        window.open(this.$store.state.youtubeChannelUrl, '_system', 'location=yes,footer=yes')
      }
    },
    goBack () {
      // 라우터 백 패스 설정
