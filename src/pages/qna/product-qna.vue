<template>
  <q-page class="">
    ...
    <div style="width: 100%; margin: 0px 0px 0px 0px;">
      <q-list separator style="padding: 0px 0px 0px 0px">
        <q-item link v-ripple.mat @click.native="goProductQnaRegister($route.query.seq)">
          <q-item-side icon="insert_comment" />
          <q-item-main label="문의하기" />
      ...
      <q-infinite-scroll :handler="loadMoreQna" ref="infiniteScrollQna">
        <div v-for="item in qnaList" :key="item.seq">
          ...
          <table border="0" cellpadding="1" cellspacing="0" style="width: 95%" align="center">
            ...
            <tr>
              <td></td>
              <td align="right"><font v-if="item.mod_time != null" size="2" color="grey">답변등록 : {{ item.mod_time }}</font></td>
            </tr>
          </table>
          <br>
        </div>
...
export default {
  ...
  methods: {
    ...
    // Q&A 등록 화면으로 이동
    goProductQnaRegister (seq) {
      // 로그인 체크
      if (!this.checkLogin()) {
        this.$refs.loginModal.show()
        return
      }
      this.$router.push('/productDetail/qnaRegister?seq=' + seq)
    },
    // 상품리뷰 loadMore
    ...
    // 컨텐츠 리스트 마지막 페이지 조회
    selectQnaListMax () {
      this.$axios.get(this.$store.state.apiServerIp + '/api/qna/selectProductQnaListLastPageNum',
        {params: {pageSize: this.pageSizeQna, seq_product: this.$route.query.seq}})
        .then((result) => {
    ...
    selectQnaList (idx, done) {
      this.$axios.get(this.$store.state.apiServerIp + '/api/qna/selectProductQnaList',
        {params: {pageNum: idx, pageSize: this.pageSizeQna, seq_product: this.$route.query.seq}})
        .then((result) => {
          // console.log(JSON.stringify(result.data))
