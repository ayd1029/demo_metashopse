  ...
  <q-page class="">
    ...
    <q-layout-header :reveal="$store.state.device !== 'P'">
      <q-toolbar color="black" inverted>
        ...
        <q-toolbar-title>상품 문의</q-toolbar-title>
    ...
    <div class="no_refresh" style="width: 100%" >
      ...
      <table width="95%" align="center" border="0">
        <tr>
          <td>
            <div v-if="updateMode">
              <q-input inverted-light color="white" v-model.trim="qnaVo.product_title" stack-label="상품명" readonly class="no-shadow" />
              <br>
            </div>
            <q-select v-model="qnaVo.qna_type" inverted-light color="white" stack-label="문의유형 선택" :options="$store.state.qnaTypeOptions" :readonly="updateMode" />
            <br>
            <div v-if="productOptionValueOptions != null && productOptionValueOptions.length > 0">
              <q-select v-model="qnaVo.product_option" inverted-light color="white" stack-label="문의상품 선택" :options="productOptionValueOptions" :readonly="updateMode" />
              <br>
            </div>
            <q-input v-model.trim="qnaVo.question" stack-label="문의내용" color="white" inverted-light type="textarea" rows="5" :readonly="updateMode" />
            <br>
            <div v-if="qnaVo.uid_to !== '' && $store.state.userVo.uid === qnaVo.uid_to">
...
export default {
  ...
  data () {
    return {
      ...
      qnaVo: {
        ...
        qna_type: '00',
        uid_to: '',
        seq_product: this.$route.query.seq,
        product_option: '',
        question: '',
        answer: '',
      ...
      productOptionValueOptions: [], // 상품 옵션 리스트
      targetFiles: [],
      targetFilesNewName: [],
      numOfTargetFiles: 0,
  ...
  created: function () {
    ...
    // 상품 옵션 리스트 조회
    this.$axios.get(this.$store.state.apiServerIp + '/api/qna/selectProductOptionValueList', {params: {seq_product: this.$route.query.seq}})
      .then((result) => {
        // console.log(JSON.stringify(result.data))
        // 상품 옵션 리스트 설정
        this.productOptionValueOptions = result.data
      })
      .catch((err) => {
        console.log(err)
      })
    if (this.$route.query.seq_qna != null) {
      ...
      this.$axios.get(this.$store.state.apiServerIp + '/api/qna/selectQna', {params: {seq: this.$route.query.seq_qna, uid_to: this.$store.state.userVo.sid}})
        .then((result) => {
  ...
  methods: {
    ...
    saveQna () {
      ...
      if (!this.updateMode) { // 등록모드
        // Q&A 등록
        this.$axios.post(this.$store.state.apiServerIp + '/api/qna/insertQna', this.qnaVo)
          .then((result) => {
            ...
            this.$router.push('/productDetail/qna?seq=' + this.$route.query.seq)
          })
          .catch((err) => {
            console.log(err)
            // this.$q.notify(err)
        ...
        this.$axios.post(this.$store.state.apiServerIp + '/api/qna/updateQna', this.qnaVo)
          .then((result) => {
            this.loading = false // 등록 버튼 액션 중단
            this.$q.notify({
              color: 'blue',
            ...
            this.$router.push('/seller/menu/sellerQnaList')
          })
          .catch((err) => {
            console.log(err)
            // this.$q.notify(err)
