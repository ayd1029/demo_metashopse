  ...
  <q-page class="">
    ...
    <q-layout-header :reveal="$store.state.device !== 'P'">
      <q-toolbar color="black" inverted>
        ...
        <q-toolbar-title>상품 수정</q-toolbar-title>
      </q-toolbar>
    </q-layout-header>
      ...
      <tr>
        <td>
          ...
          <div v-if="divSellerCode">
            <br>
            <q-input v-model.trim="productVo.seller_product_cd" stack-label="판매자 상품코드" maxlength="100" clearable />
            <br>
            <q-input v-model.trim="productVo.seller_barcode" stack-label="판매자 바코드" maxlength="100" clearable />
            <br>
            <q-input v-model.trim="productVo.seller_inner_cd1" stack-label="판매자 내부코드1" maxlength="100" clearable />
            <br>
            <q-input v-model.trim="productVo.seller_inner_cd2" stack-label="판매자 내부코드2" maxlength="100" clearable />
          </div>
          <br>
          <q-toggle v-model="productVo.free_delivery_yn" true-value="true" false-value="false" readonly>&nbsp;&nbsp;&nbsp;무료배송 (기본설정)</q-toggle>
          <br>
          <br>
          <q-toggle v-model="productVo.dawn_delivery_yn" true-value="true" false-value="false">&nbsp;&nbsp;&nbsp;새벽배송</q-toggle>
          <br>
          <br>
...
export default {
  ...
  created: function () {
    ...
    this.$axios.get(this.$store.state.apiServerIp + '/api/product/selectProduct',
      {params: {seq: seqProduct}})
      .then((result) => {
        // console.log(JSON.stringify(result.data))
        this.productVo = result.data

        // 옵션 설정
        this.selectOptionCount = this.productVo.select_option_count
        this.selectOptionType = this.productVo.select_option_type
        this.directOptionCount = this.productVo.direct_option_count

        this.selectOptionList = this.productVo.productOptionList

        // divProductOption, divSelectOption, divDirectOption 컨트롤
        if (this.selectOptionCount > 0 || this.directOptionCount > 0) {
          this.divProductOption = true
        }
        if (this.selectOptionCount > 0) {
          this.divSelectOption = true
          this.selectOptionName1 = this.productVo.select_option_name1
          this.selectOptionValue1 = this.productVo.select_option_value1
          this.selectOptionName2 = this.productVo.select_option_name2
          this.selectOptionValue2 = this.productVo.select_option_value2
          this.selectOptionName3 = this.productVo.select_option_name3
          this.selectOptionValue3 = this.productVo.select_option_value3
        } else {
          this.selectOptionCount = '1' // 디폴트
          this.selectOptionType = '1' // 디폴트 단독형
        }
        if (this.directOptionCount > 0) {
          this.divDirectOption = true
          this.directOptionName1 = this.productVo.direct_option_name1
          this.directOptionName2 = this.productVo.direct_option_name2
          this.directOptionName3 = this.productVo.direct_option_name3
          this.directOptionName4 = this.productVo.direct_option_name4
          this.directOptionName5 = this.productVo.direct_option_name5
        } else {
          this.directOptionCount = '1' // 디폴트
        }
        ...
        // 판매자코드 설정 활성화 체크
        if (this.productVo.seller_product_cd !== '' ||
            this.productVo.seller_barcode !== '' ||
            this.productVo.seller_inner_cd1 !== '' ||
            this.productVo.seller_inner_cd2 !== '') {
          this.divSellerCode = true
        }
      })
      .catch((err) => {
        console.log(err)
  ...
  methods: {
    sumQuantity () { // 옵션 선택형 조합형 재고수량 변경시 총 수량 계산 후 설정
      ...
      for (let i = 0; i < this.selectOptionList.length; i++) {
        sumQuantity += Number(this.selectOptionList[i].option_quantity)
      }
    ...
    */
    ...
    updateProduct () {
      ...
      // 상품 수정
      this.$axios({
        url: this.$store.state.apiServerIp + '/api/product/updateProduct',
        method: 'POST',
        data: this.productVo })
        .then((result) => {
          this.loading = false // 수정 버튼 액션 중단
          this.$q.notify({
            color: 'info',
            position: 'top',
            message: '상품이 수정되었습니다.',
          ...
          this.$router.push('/seller/product/productDetail?seq=' + this.$route.query.seq)
        })
        .catch((err) => {
          console.log(err)
          // this.$q.notify(err)
