  ...
  <q-page class="">
    ...
    <q-layout-header :reveal="$store.state.device !== 'P'">
      <q-toolbar color="black" inverted>
        ...
        &nbsp;&nbsp;&nbsp;
        <q-toolbar-title><font size="4"><b>{{ userVo.name }}</b></font></q-toolbar-title>
        <q-btn v-if="hasOpener" flat round dense icon="close" @click="close" />
    ...
    <div style="width: 100%;">
      <table style="width: 100%" border="0">
        ...
        <tr>
          <td colspan="2"><b><font size="2" color="black">&nbsp;&nbsp;{{ userVo.name }}</font></b></td>
          <td colspan="4" align="right">
            <!-- 팔로잉이면 SNS 링크 버튼, 아니면 팔로우 버튼 표시 -->
            <div v-if="userVo.follow_yn === 'Y'">
              ...
              <q-btn v-if="userVo.uid !== $store.state.userVo.uid" dense color="grey" label="팔로잉" @click="setFollow(userVo.uid, 'OFF')" style="width:65px;" />&nbsp;
            </div>
            <div v-else><q-btn v-if="userVo.uid !== $store.state.userVo.uid" dense color="blue" label="팔로우" @click="setFollow(userVo.uid, 'ON')" style="width:65px;" />&nbsp;</div>
          </td>
        </tr>
      </table>
      ...
      <div class="row justify-center q-mt-lg">
        <q-chip color="primary" square icon="widgets">
          {{ userVo.name }} 님이 좋아하는 상품
        </q-chip>
      </div>
      <br>
      -->
      <div style="width:100%">
        <q-card-separator />
        <q-tabs v-model="selectedHomeTab" inverted align="justify" color="primary">
              ...
              <div style="padding:0px 0px 0px 0px" v-for="item in productList" :key="item.seq">
                <q-item :to="`/productDetail?seq=${item.seq_product}&uid=${uid}`" style="padding:0px 0px 0px 0px">
                  <table border="0" style="width: 100%; padding:5px 0px 5px 0px;">
                    ...
                    <tr>
                      <td colspan="5" style="text-align: right;"><font size="1" color="orange">★★★★☆ (38)&nbsp;</font></td>
                    </tr>
                  </table>
                </q-item>
              </div>
...
export default {
  ...
  data () {
    return {
      ...
      hasOpener: true, // 팝업 표시면 우상단에 닫기 버튼을, 링크면 홈 버튼을 표시
      selectedHomeTab: 'CONTENTS', // 디폴트 탭
      // loading: false,
      // percentage: 0,
      refresherDone: '',
      // offset: 500,
  ...
  watch: {
    '$route.query.uid': function (uid) {
      this.uid = uid

      // 마이스토어 주소 설정
      this.mystoreUrl = this.$store.state.DOMAIN + '/home/' + uid

      // 마이스토어 정보 조회
      this.selectMystoreInfo(uid)
  ...
  created: function () {
    // 팝업 표시면 우상단에 닫기 버튼을, 링크면 홈 버튼을 표시
    if (window.opener == null) {
      this.hasOpener = false
    }
    console.log('hasOpener: ' + this.hasOpener)
    ...
    // 마이스토어 주소 설정
    this.mystoreUrl = this.$store.state.DOMAIN + '/home/' + this.uid

    // 마이스토어 정보 조회
    this.selectMystoreInfo(this.uid)
  },
  mounted: function () {
    // 디폴트 리스트 설정 - 컨텐츠 리스트
    this.setDefaultList()
  ...
  methods: {
    ...
    refresher (done) {
      ...
      console.log('this.selectedHomeTab: ' + this.selectedHomeTab)
      if (this.selectedHomeTab === 'CONTENTS') {
        this.$refs.infiniteScrollContents.reset() // index 초기화
        this.$refs.infiniteScrollContents.resume() // stop에서 다시 재생
        this.$refs.infiniteScrollContents.loadMore() // loadMore로 검색
      } else if (this.selectedHomeTab === 'PRODUCT') {
        this.$refs.infiniteScrollProduct.reset() // index 초기화
        this.$refs.infiniteScrollProduct.resume() // stop에서 다시 재생
        this.$refs.infiniteScrollProduct.loadMore() // loadMore로 검색
      } else if (this.selectedHomeTab === 'CIRCLE') {
        this.$refs.infiniteScrollCircle.reset() // index 초기화
        this.$refs.infiniteScrollCircle.resume() // stop에서 다시 재생
        this.$refs.infiniteScrollCircle.loadMore() // loadMore로 검색
      } else {
        console.log('this.selectedHomeTab in refresher is else..')
      }
    },
    // 상품 상세 화면으로 이동
    ...
    selectContentsListMax () {
      this.$axios.get(this.$store.state.apiServerIp + '/api/contents/selectUserContentsListLastPageNum',
        {params: {pageSize: this.pageSizeContents, uid: this.uid}})
        .then((result) => {
          // console.log(JSON.stringify(result.data))
    ...
    selectContentsList (idx, done) {
      this.$axios.get(this.$store.state.apiServerIp + '/api/contents/selectUserContentsList',
        {params: {pageNum: idx, pageSize: this.pageSizeContents, uid: this.uid}})
        .then((result) => {
          // console.log(JSON.stringify(result.data))
    ...
    setFollow (uidTo, onOff) {
      ...
      followVo.uid_to = uidTo

      if (onOff === 'ON') { // 팔로우
        this.userVo.follow_yn = 'Y' // 팔로우
        this.userVo.count_follow_from = Number(this.userVo.count_follow_from) + 1
    ...
    close () {
      ...
      window.close()
      /*
      if (this.$q.platform.is.platform === 'android') { // 안드로이드 앱인 경우
